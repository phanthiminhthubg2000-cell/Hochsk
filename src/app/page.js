...
const CircleProgress = ({ percent, size = 120, strokeWidth = 10, children }) => {
  ...
  return (
    <div className="relative flex items-center justify-center drop-shadow-sm" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="-rotate-90 transform" viewBox={`0 0 ${size} ${size}`}>
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke="#EEF5E9" strokeWidth={strokeWidth} />
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke="#2F8F6E" strokeWidth={strokeWidth} strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={offset} style={{ transition: "stroke-dashoffset 1s ease-in-out" }} />
      </svg>
...
export default function HomePage() {
  ...
  const getHskIcon = (progress, isCurrent) => {
    if (progress === 100 && !isCurrent) return '🌸';
    if (isCurrent) return '✨';
  ...
  // Khu vườn cá nhân hóa (Màu trơn - theo hình 3)
  const gardenAreas = [
    { name: "Cây Từ vựng", level: Math.floor((skillMap?.vocabulary || 40) / 10) + 1, icon: "🌱", link: "/vocab", bg: "bg-[#2F8F6E]", text: "text-white" },
    { name: "Đầm Chủ đề", level: Math.floor(((skillMap?.vocabulary || 40) + (skillMap?.translation || 40)) / 20) + 1, icon: "🪷", link: "/topic", bg: "bg-[#F2765B]", text: "text-white" },
    { name: "Hoa Ngữ pháp", level: Math.floor((skillMap?.grammar || 30) / 10) + 1, icon: "☀️", link: "/arrange", bg: "bg-[#FFD666]", text: "text-[#1B5E4B]" },
    { name: "Ao Nghe", level: Math.floor((skillMap?.listening || 30) / 10) + 1, icon: "💧", link: "/dictation", bg: "bg-[#4FB6C7]", text: "text-white" },
    { name: "Gió Dịch", level: Math.floor((skillMap?.translation || 30) / 10) + 1, icon: "🍃", link: "/translate", bg: "bg-[#8FD9A8]", text: "text-[#1B5E4B]" },
    { name: "Sân HSKK", level: Math.floor((skillMap?.speaking || 30) / 10) + 1, icon: "🎤", link: "/roleplay", bg: "bg-[#A97845]", text: "text-white" },
  ];
  ...
  return (
    <div className="min-h-screen font-sans text-[#1B5E4B] relative bg-[#EEF5E9] selection:bg-[#8FD9A8]/50">
      ...
      {/* ==========================================
          SIDEBAR
          ========================================== */}
      <aside className={`fixed inset-y-0 left-0 z-40 hidden flex-col border-r border-[#8FD9A8]/30 bg-[#F7FAF3] transition-all duration-300 md:flex ${isSidebarCollapsed ? "w-[76px]" : "w-[240px]"}`}>
      ...
      <main className={`min-h-screen transition-all duration-300 relative z-10 ${isSidebarCollapsed ? "md:pl-[76px]" : "md:pl-[240px]"}`}>
        ...
        <header className="sticky top-0 z-30 h-[76px] border-b border-[#8FD9A8]/30 bg-[#EEF5E9]/80 px-5 backdrop-blur-xl md:px-8 flex items-center justify-between">
          <button onClick={() => setIsSearchOpen(true)} className="flex h-11 max-w-md flex-1 items-center gap-2 rounded-2xl bg-white shadow-sm px-4 text-left text-sm font-medium text-slate-400 hover:shadow-md transition-all sm:flex group border border-transparent hover:border-[#8FD9A8]">
          ...
          <div className="ml-auto flex items-center gap-3">
            <div className="flex items-center gap-1.5 rounded-2xl bg-white shadow-sm px-4 py-2.5 border border-[#E2E8F0]">
              <span className="text-lg drop-shadow-sm">🔥</span><span className="text-xs font-black text-[#F2765B]">{streak}</span>
            </div>
            <div className="flex items-center gap-1.5 rounded-2xl bg-[#4FB6C7]/10 border border-[#4FB6C7]/30 shadow-sm px-4 py-2.5">
              <span className="text-lg drop-shadow-sm">💧</span><span className="text-xs font-black text-[#4FB6C7]">{water}</span>
            </div>
        ...
        <div className="mx-auto max-w-6xl space-y-8 px-5 py-8 md:px-8 pb-20">
          ...
          {/* ==========================================
              HERO BANNER
              ========================================== */}
          <section className="relative rounded-[32px] overflow-hidden shadow-md bg-[#1B5E4B] p-8 md:p-12 text-white flex flex-col md:flex-row justify-between items-center min-h-[260px]">
            ...
            <div className="absolute -left-20 -top-20 w-96 h-96 bg-[#2F8F6E] rounded-full blur-[80px] pointer-events-none opacity-50"></div>
            
            <div className="relative z-10 w-full md:w-2/3">
               <h2 className="text-[10px] font-black text-[#8FD9A8] uppercase tracking-widest mb-3">🌿 Hôm nay trong khu vườn</h2>
               <h1 className="text-3xl md:text-4xl font-black tracking-tight mb-2 leading-[1.2] text-white drop-shadow-sm">
                  Chào {user?.firstName || user?.fullName || "bạn"} 👋
               </h1>
               <p className="text-sm md:text-base text-[#8FD9A8] font-medium max-w-md leading-relaxed mb-6">
               ...
               <div className="mb-8 w-full max-w-sm">
                 ...
                 <div className="h-2.5 bg-white/20 rounded-full overflow-hidden shadow-inner">
                   <div className="h-full bg-[#FFD666] rounded-full transition-all duration-1000 shadow-sm" style={{ width: `${currentLevelProgress}%` }}></div>
                 </div>
               </div>

               <div className="flex flex-wrap items-center gap-3">
                  <Link href="/vocab">
                    <button className="bg-white text-[#1B5E4B] px-7 py-3 rounded-[18px] text-sm font-black hover:bg-[#8FD9A8] hover:text-[#1B5E4B] transition-all shadow-lg flex items-center gap-2 hover:-translate-y-0.5">
                      🌱 Học tiếp
                    </button>
                  </Link>
                  <a href="#quest">
                    <button className="bg-white/10 text-white border border-white/20 px-6 py-3 rounded-[18px] text-sm font-bold hover:bg-white/20 transition-all flex items-center gap-2">
                      🎯 Xem nhiệm vụ
                    </button>
                  </a>
               </div>
            ...
            <div className="hidden md:flex relative z-10">
               <span className="text-[130px] drop-shadow-[0_20px_40px_rgba(0,0,0,0.3)] animate-[float_4s_ease-in-out_infinite] cursor-pointer hover:scale-105 transition-transform">
                 {streak < 3 ? '😴' : streak < 7 ? '🐸' : streak < 15 ? '😎' : '👑'}
               </span>
               <div className="absolute -bottom-4 left-1/2 transform -translate-x-1/2 w-20 h-4 bg-black/20 blur-md rounded-full"></div>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {gardenAreas.map((tool, index) => (
                <Link href={tool.link} key={index} className={`group relative rounded-[24px] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 h-[140px] flex flex-col justify-end ${tool.bg} ${tool.text}`}>
                  <div className={`absolute top-4 right-4 flex h-10 w-10 items-center justify-center rounded-[40%_60%_70%_30%/40%_50%_60%_50%] bg-white/20 backdrop-blur-md text-xl shadow-sm transition-transform group-hover:scale-110`}>{tool.icon}</div>
                  <div className="relative z-10 p-4">
                    <h3 className="text-sm font-black tracking-wide drop-shadow-sm leading-tight">{tool.name}</h3>
                    <p className="mt-1 text-[10px] font-bold opacity-90 border border-white/40 bg-white/10 px-2 py-0.5 rounded-md inline-block backdrop-blur-sm shadow-sm">Cấp {tool.level}</p>
                  </div>
                </Link>
              ))}
            </div>
          ...
          {/* ==========================================
              HÀNH TRÌNH TRỒNG SEN (HSK 1 - 9) & NHIỆM VỤ NGÀY
              ========================================== */}
          <section className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]" id="quest">
            ...
            <div className="rounded-[32px] bg-white p-8 shadow-sm border border-[#E2E8F0]">
              <div className="mb-8 flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-black text-[#1B5E4B] flex items-center gap-2"><span>🪷</span> Lộ trình sinh trưởng</h2>
                </div>
              ...
              <div className="relative py-4 flex flex-col-reverse gap-4 before:absolute before:left-[23px] before:top-6 before:bottom-6 before:w-[4px] before:bg-[#8FD9A8]/30 before:rounded-full z-0">
                {hskLevels.map((item) => {
                  ...
                  const iconNode = getHskIcon(item.progress, isCurrent);
                      ...
                      <div className={`flex-1 rounded-[24px] p-4 transition-all ${isCurrent ? "bg-[#FDFBF7] shadow-sm border border-[#FFD666]/40 translate-x-1" : "bg-transparent group-hover:bg-slate-50 group-hover:shadow-sm border border-transparent group-hover:border-[#E2E8F0]"}`}>
                        <div className="mb-2 flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-[10px] font-bold text-[#2F8F6E]/70 mt-0.5">{item.title} · {item.words.toLocaleString("en-US")} từ</p>
                          </div>
                          <span className={`text-[10px] font-black px-3 py-1 rounded-xl ${isCurrent ? 'bg-[#FFD666] text-[#1B5E4B]' : 'bg-[#F1F5F9] text-slate-400'}`}>{item.progress}%</span>
                        </div>
                        <div className="h-1.5 overflow-hidden rounded-full bg-[#E2E8F0]/60">
            ...
            <div className="rounded-[32px] bg-white p-8 shadow-sm flex flex-col gap-4 border border-[#E2E8F0]">
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-xl font-black text-[#1B5E4B] flex items-center gap-2"><span>🌞</span> Nhiệm vụ hôm nay</h2>
                <div className="w-10 h-10 rounded-full border-[3px] border-[#8FD9A8] text-[#2F8F6E] flex items-center justify-center text-[11px] font-black">
              ...
              {dailyMissions.map((mission, index) => {
                ...
                return (
                  <div key={index} className={`rounded-[24px] border p-4 flex flex-col gap-3 transition-all cursor-pointer group ${isDone ? 'bg-[#EEF5E9]/50 border-[#8FD9A8]/50 shadow-sm' : 'bg-white border-[#E2E8F0] hover:border-[#8FD9A8]'}`}>
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg transition-colors ${isDone ? 'bg-[#2F8F6E] text-white shadow-inner' : 'bg-[#F4F7F6] text-[#2F8F6E]'}`}>
                        {isDone ? '✅' : mission.icon}
                      </div>
                      <div className="flex-1">
                        <h4 className={`font-bold text-sm ${isDone ? 'text-[#2F8F6E]' : 'text-[#1B5E4B]'}`}>{mission.title}</h4>
                        <span className="text-[10px] font-black text-[#F2765B] mt-0.5 inline-block">+{mission.xp} XP & +5 💧</span>
                      </div>
                      {!isDone && (
            ...
            <div className="rounded-[32px] bg-white p-8 shadow-sm border border-[#E2E8F0]">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                 ...
                 <div className="flex flex-col gap-4">
                    <div className="bg-[#EEF5E9] p-4 rounded-2xl border border-[#8FD9A8]/50">
                    ...
                    <div className="bg-[#F2765B]/5 p-4 rounded-2xl border border-[#F2765B]/30">
          ...
          <div className="relative flex max-h-[85vh] w-full max-w-3xl flex-col overflow-hidden rounded-[32px] bg-white shadow-2xl">
            <div className="flex flex-col border-b border-[#E2E8F0] p-5">
              <div className="flex items-center">
                ...
                <button onClick={closeSearch} className="rounded-xl bg-[#F2765B]/10 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-[#F2765B] hover:bg-[#F2765B] hover:text-white">ĐÓNG</button>
              </div>
            </div>
